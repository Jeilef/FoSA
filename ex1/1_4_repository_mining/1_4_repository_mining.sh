...
join <(git log --max-parents=1 --format="" --since="1 year ago" --name-only | sort | uniq) <(git ls-files | sort) | wc -l | awk -v total=$(git ls-files | sort | wc -l) '{printf "Share of stale code: %.1f%\n", (1-($1/total)) * 100}'
git log --format="%s" --since="1 month ago" | tr " " "\n" |  tr '[:upper:]' '[:lower:]' | grep -ivwE "a|an|at|for|from|in|is|of|on|the|to|use|with|when|[^#[:alnum:]]+" | sed -E 's/[^#[:alnum:]]+//g' | sort | uniq -c | sort -ri | head -n 10 | awk '{print $2}' | tr "\n" "," | sed 's/.$//' | awk '{print "Top 10 commit message keywords last month: "$1}'